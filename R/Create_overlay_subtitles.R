...
Create_overlays_subtitle <- function(
  to.data = par_to.data(),
  traj.data,
  raw.avi.folder = par_raw.video.folder(),
  ...
  traj.data$subtitle <- paste0(
    "Dialogue: 2,0:00:",
    sprintf("%05.2f", traj.data$starttime), ",0:00:",
    sprintf("%05.2f", traj.data$endtime),
    ",Numbers,,0000,0000,0000,,{\\pos(",
    abs(round(traj.data$X)), ", ", abs(round(traj.data$Y)), ")}",
    traj.data$ID.vid
  )
  traj.data$subtitle2 <- paste0(
    "Dialogue: 1,0:00:",
    sprintf("%05.2f", traj.data$starttime), ",0:00:",
    sprintf("%05.2f", traj.data$endtime), ",Circle,,0000,0000,0000,,{\\pos(",
    abs(round(traj.data$X)), ", ",
    ifelse(
      abs(round(traj.data$Y)) < height,
  ...
  header <- c(
    "[Script Info]",
    "ScriptType: v4.00+", "Collisions: Normal", paste0("PlayResX: ", width), paste0("PlayResY: ", height),
    paste0("Timer: ", vid.length), "\r",
    "[V4+ Styles]",
  ...
  # For each of the files with observed cells, burn the subtitles onto the avi file,
  # and store the resulting file in the overlay folder
  for (i in unique(traj.data$file)) {
    avi <- grep(i, avi.files, value = TRUE)
    ssa <- grep(i, ssa.files, value = TRUE)
